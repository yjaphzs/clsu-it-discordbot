    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if (interaction.commandName == "promote") {
            ...
            if (!(interaction.member &&
                ...
                interaction.member.permissions.has(discord_js_1.PermissionsBitField.Flags.Administrator))) {
            ...
            if (mentionMatch) {
                ...
                if (promoted) {
                    ...
                    return interaction.editReply(`<@${member.user.id}> could not be promoted. They may already be at the highest year level or do not meet the requirements. ⚠️`);
                }
            }
            ...
            if (!role) {
                return interaction.reply({
                    content: "Fourth Year role not found. Please check the role ID.",
                    flags: discord_js_1.MessageFlags.Ephemeral,
                });
            }
            ...
            if (alumniMessage) {
                ...
                collector.on("collect", async (reaction, user) => {
                    try {
                        ...
                        if (alumniMessage.channel &&
                            alumniMessage.channel.isTextBased() &&
                            "send" in alumniMessage.channel) {
                            await alumniMessage.channel.send(`An error occurred while assigning the <@&${ALUMNI_ROLE_ID}> role. Please contact an administrator. ❌`);
                        }
                    }
                });
            }
